...
class TransferTable extends PureComponent {
    ...
    loadLeftData = (value,pageInfo) => {
        ...
        leftService({...value,...pageInfo}).then(res => {
            this.setState({leftData: res instanceof Array?res:[],leftLoading:false,leftRowsSelected:[],rightDisabled:true,leftDisabled:true})
        }).catch(err => {
    ...
    loadRightData = (value,pageInfo) => {
        ...
        rightService({...value,...pageInfo}).then(res => {
            this.setState({rightData: res instanceof Array?res:[],rightLoading:false,rightRowsSelected:[],rightDisabled:true,leftDisabled:true})
        }).catch(err => {
    ...
    render() {
        ...
        const leftTitle = () =>{
            return[
                treeSelectConfig&&<span style={{marginLeft:5}}  key={"treeSelecteWithServiceLable"}>{treeSelectConfig.lable?treeSelectConfig.lable+":":""}</span>,
                treeSelectConfig&&<span style={{marginLeft:5}} key={"treeSelecteWithService"}><TreeSelectWithService config={treeSelectConfig} width={220} onChange={this.selectedWithServiceChange}/></span>,

                checkBoxConfig&&<span style={{marginLeft:5}}  key={"checkboxLable"}>{checkBoxConfig.lable?checkBoxConfig.lable+":":""}</span>,
                checkBoxConfig&&<span style={{marginLeft:5}}  key={"checkbox"}><Checkbox onChange={this.includeSubNode} style={{marginLeft:5}}/></span>,

                searchTableConfig&&<span style={{marginLeft:5}}  key={"searchTableLable"}>{searchTableConfig.lable?searchTableConfig.lable+":":""}</span>,
                searchTableConfig&&<span style={{marginLeft:5}}  key={"searchTable"}>
                       <SearchTable isNotFormItem={true} config={searchTableConfig} initValue={false} selectChange={this.selectChange} style={{width:220,marginLeft:5}} />
                    </span>
        ...
        return (
                <Row style={{margin: '0px 8px',position:'absolute',
                    ...
                    <Col key='left' span={11}>
                        ...
                        <Row style={{background:'#F3F8FC',paddingTop:10,paddingBottom:5}}>
                            <Col span={16}>{<span>{leftTitle()}</span>}</Col>
                            <Col span={8} style={{textAlign:'right'}}>{leftSearch()}</Col>
                        </Row>
                        <SimpleTable
                            checkBox
