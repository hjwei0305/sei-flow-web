...
class StandardTree extends Component {
  ...
  componentDidMount() {
   ...
   wrappedScroller = () => {
      ...
      this.ps = new PerfectScrollbar('#custom-tree');
      this.ps && this.ps.update();
    }
  ...
  render() {
    return (
      <div>
        ...
        <div ref={(div) => this.simpleDiv = div} id={"custom-tree"} style={{position: "relative"}}>
          {this.state.dadaSource.length > 0 ? (
            <Card style={{height: this.state.scrollY + 82}} bordered={false} bodyStyle={{padding: "0 0px 0px 30px"}}>
              <DirectoryTree
