...
import {WorkFlow, message} from 'suid';
...
const {defaultPageSize, rowGutter,} = constants;
const {seiIntl} = seiLocale;

const Search = Input.Search;
const confirm = Modal.confirm;

class BusinessModel2ViewSuid extends Component {
  constructor(props) {
    super(props);
    this.state = {
      treeData: [],
          ...
          let ops = () => {
            ...
            if ("INIT" == record.flowStatus) {
              const startProps = {
                ...
                startComplete: this.getPageData,
              };
              ...
              ops.push(<a className={'row-operator-item'} key={"delete" + index}
                          onClick={() => this.onDeleteClick(record)}>{seiIntl.get({
                key: 'flow_000032',
                desc: '删除'
              })}</a>);
              ops.push(<StartFlow {...startProps} />);
            }
