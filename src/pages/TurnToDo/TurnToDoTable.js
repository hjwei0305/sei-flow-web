...
class TurnToDoTable extends Component {
  ...
  render() {
    const columns = [
      {
        ...
        render: (text, record, index) => {
          let ops = [
            ...
            }}>{seiIntl.get({key: 'flow_000046', desc: '转办'})}</a>
          ]
          return (
      ...
      {
        title: seiIntl.get({key: 'flow_000325', desc: '执行人'}),
        dataIndex: 'executorName',
        width: 200,
        render: (text, record, index) => {
          if (record) {
            const res = `${record.executorName}【${record.executorAccount}】`;
    ...
    const search = () => {
      return [
        <Tooltip title={seiIntl.get({key: 'flow_000324', desc: '流程名称、任务名称、业务单号、执行人、描述'})}>
          <Search
            key="search"
