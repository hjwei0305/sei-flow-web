  ...
  return this.each( function() {
    ...
    populate = function (npage) {
      if(!ts.grid.hDiv.loading) {
        ...
        case "xml":
        case "script":
          var dataParams = $.isFunction(ts.p.serializeGridData)? ts.p.serializeGridData.call(ts,ts.p.postData) : ts.p.postData;
          if (ts.p.isJson) {
          ...
          $.ajax($.extend({
            ...
            beforeSend: function(xhr, settings ){
              // xhr.setRequestHeader("Authorization", ts.p.headers['Authorization']);
              var gotoreq = true;
    ...
    setPager = function (pgid, tp){
      ...
      if(dir==="rtl") { pgl += str; }
      if(ts.p.pginput===true) {
        pginp= "<td id='input"+tp+"' dir='"+dir+"'>"+$.jgrid.template( $.jgrid.getRegional(ts, "defaults.pgtext", ts.p.pgtext) || "",
