...
EUI.LookWorkFlowView = EUI.extend(EUI.CustomUI, {
  ...
  openShowWin: function (nodeName, executorList) {
    ...
    g.win = EUI.Window({
      ...
      items: [{
        ...
        padding: 0,
        itemspace: 0,
        items: [this.initExecutorWind(nodeName, executorList)]
      }]
    });
  },
  initExecutorWind: function (nodeName, executorList) {
    return {
      xtype: "GridPanel",
      border: true,
      title: nodeName,
  ...
  showTaskNode: function (id, node, currentNodes) {
    ...
    var solidifyCss = "";
    if (this.solidifyExecutorsInfo.length > 0) {
      solidifyCss += " solidifyExecutorLook";
    }

    //工作池任务，服务任务，接收任务  不加载查看执行人按钮
    if (node.nodeType == "ServiceTask" || node.nodeType == "PoolTask" || node.nodeType == "ReceiveTask") {
