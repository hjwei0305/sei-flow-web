...
EUI.WorkFlowMoreInfoView = EUI.extend(EUI.CustomUI, {
    ...
    checkAllowSolidFlow: function(){
        ...
        if (nodes && nodes.length > 0) {
            for (var i = 0; i < nodes.length; i++) {
                ...
                if(type==="ServiceTask"||type==="CallActivity"|| type==="PoolTask"){
                    EUI.ProcessStatus({
                        msg: "固化流程不能包含【服务任务】或【调用子流程】或【工作池任务】，请检查！",
                        success: false
                    });
                    return false;
                }
    ...
    doExcludeNodes: function(solidifyFlow){
        var nodeService = $(".flow-task-box>[nodetype='ServiceTask']").parent(),
            nodeCallActivity = $(".flow-task-box>[nodetype='CallActivity']").parent(),
            nodePoolTask = $(".flow-task-box>[nodetype='PoolTask']").parent();
        if(solidifyFlow){
            nodeService&&nodeService.hide();
            nodeCallActivity&&nodeCallActivity.hide();
            nodePoolTask&&nodePoolTask.hide();
        }else{
            nodeService&&nodeService.show();
            nodeCallActivity&&nodeCallActivity.show();
            nodePoolTask&&nodePoolTask.show();
        }
    },
    show: function () {
        this.form.show();
