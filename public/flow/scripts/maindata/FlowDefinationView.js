...
EUI.FlowDefinationView = EUI.extend(EUI.CustomUI, {
    ...
    initWindTbar: function (oldRowDatas) {
        var g = this;
        return [{
                xtype: "Button",
                ...
                selected:true,
                handler: function() {
                    var orgName = oldRowDatas.orgName;
                    var rowData =EUI.getCmp("defViesonGridPanel").getSelectRow();
    ...
    copyFlowDefination: function (data,isFromVersion) {
        ...
        var versionCode=isFromVersion?data["versionCode"]:null;
        //isFromVersion是判断参考创建有没有组织机构，流程类型是否可以重新选择（外面参考创建和版本里面参考创建）
        var url=_ctxPath + "/design/show?orgId=" + g.selectedNodeId +"&orgCode="+data.orgCode+"&orgName="+encodeURIComponent(encodeURIComponent(data.orgName))+"&id="+id+"&businessModelId="+businessModelId+"&businessModelCode="+businessModelCode+"&isCopy="+true+"&isFromVersion="+false;
        if(isFromVersion==true&&versionCode!=null){
    ...
    resetPosition : function (data){
        var g = this;
        var infoBox = EUI.MessageBox({
            title: g.lang.tiShiText,
            msg: g.lang.resetPositionHintMessageText,
            buttons: [{
                title: g.lang.cancelText,
                handler: function () {
                    infoBox.remove();
                ...
                handler: function () {
                    infoBox.remove();
                    var myMask = EUI.LoadMask({
                        msg: g.lang.resetPositionMaskMessageText
                    });
                    EUI.Store({
                        url: _ctxPath + "/design/resetPosition",
                        params: {
                            id: data.id
                        },
                        success: function (result) {
    ...
    updateFlowDefVersion: function (data) {
        ...
        var tab = {
            ...
            url: _ctxPath + "/design/show?orgId=" + g.selectedNodeId +"&orgCode="+data["flowDefination.orgCode"]+"&id="+data["flowDefination.id"]+"&businessModelId="+data["flowDefination.flowType.businessModel.id"]+"&businessModelCode="+data["flowDefination.flowType.businessModel.className"]+"&versionCode="+data["versionCode"],
        };
        g.addTab(tab);
    },
    addTab: function (tab) {
    ...
    initGridTbar: function(){
        ...
        return [{
            ...
            xtype: "Button",
            title: g.lang.resetPositionText,  //位置重置
            iconCss:"ecmp-eui-setting",
            handler: function() {
                var rowData =EUI.getCmp("gridPanel").getSelectRow();
                if (rowData && rowData.id) {
                    g.resetPosition(rowData);
                } else {
                    EUI.ProcessStatus({msg:g.lang.resetMessage,success: false});
                }
